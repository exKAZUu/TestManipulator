...
namespace TestManipulator {
	public class ProgramForRemovingTestCases {
		public static void Do(string[] args) {
			...
			while (true) {
				...
				var allPathAndIndicies = paths.SelectMany(
						path => sources[path].IndicesOf("@Test")
				...
				// Remove @Test
				var insertedDummies = new HashSet<string>();
				foreach (var pathAndIndex in pathAndIndicies) {
					...
					var testIndex = source.IndexOf("@Test", sourceIndex);
					var insertText = "//";
					if (!insertedDummies.Contains(path)) {
						insertedDummies.Add(path);
						insertText = "@Test public void testDummy() {} //";
					}
					sources[path] = source.Substring(0, testIndex) + insertText
		...
		private static Dictionary<string, string> GetCleanedSources(List<string> paths) {
			var sources = paths.Select(
					path => {
						...
						source = source
								.Replace("@Test public void testDummy() {} //@Test", "@Test")
								.Replace("//@Test", "@Test");
